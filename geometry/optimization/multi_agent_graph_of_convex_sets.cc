...
// using AgentData = GraphOfConvexSets::Edge::AgentData;
...
// Multi-agent case modification for class Edge
// LIZHUANG Modified here
...
// Old interface for single-agent case
VectorXDecisionVariable Edge::NewSlackVariables(
  int rows, const std::string& name = "s") {
    return NewSlackVariables(0, rows, name);
  }
...
Binding<Cost> Edge::AddCost(
    const Binding<Cost>& binding,
    const std::unordered_set<Transcription>& use_in_transcription) {
  return AddCostForAgent(0, binding, use_in_transcription);
}
...
Binding<Constraint> Edge::AddConstraint(
    ...
    const std::unordered_set<GraphOfConvexSets::Transcription>&
        use_in_transcription) {
  return AddConstraintForAgent(0, binding, use_in_transcription);
}
...
void Edge::AddPhiConstraint(bool phi_value) {
  AddPhiConstraintForAgent(0, phi_value);
}
...
// Old interface for multi-agent case
void Edge::ClearPhiConstraints() {
  ClearPhiConstraintsForAgent(0);
}
...
std::optional<double> Edge::GetSolutionCost(
  const MathematicalProgramResult& result) const {
  return GetSolutionCostForAgent(result, 0);
}
...
std::optional<double> Edge::GetSolutionCost(
  ...
  return GetSolutionCostForAgent(result, cost, 0);
}
...
std::optional<Eigen::VectorXd> Edge::GetSolutionPhiXu(
    const solvers::MathematicalProgramResult& result) const {
  return GetSolutionPhiXuForAgent(result, 0);
}

std::optional<Eigen::VectorXd> Edge::GetSolutionPhiXv(
    const solvers::MathematicalProgramResult& result) const {
  return GetSolutionPhiXvForAgent(result, 0);
}
...
//----- LIZHUANG Modified Here -----
Vertex* GraphOfConvexSets::AddVertex(const ConvexSet& set, std::string name, int n_agents) {
  if (name.empty()) {
    name = fmt::format("v{}", vertices_.size());
  }
  VertexId id = VertexId::get_new_id();
...
// ------LIZHUANG ADDED HERE------
Edge* GraphOfConvexSets::AddEdge(Vertex* u, Vertex* v, std::string name, int n_agents) {
  DRAKE_DEMAND(u != nullptr && IsValid(*u));
  DRAKE_DEMAND(v != nullptr && IsValid(*v));
  if (name.empty()) {
    name = fmt::format("e{}", edges_.size());
...
std::string GraphOfConvexSets::GetGraphvizStringForAgent(
    const solvers::MathematicalProgramResult* result,
    const GcsGraphvizOptions& options,
    int agent,
    const std::vector<const Edge*>* active_path) const {
...
std::unique_ptr<MathematicalProgram>
GraphOfConvexSets::ConstructPreprocessingProgramForMultiAgent(
...
// Overloaded version of PreprocessShortestPath for multi-agent case -- Lizhuang Modified.
std::set<EdgeId> GraphOfConvexSets::PreprocessShortestPathForMultiAgent(
    const std::vector<VertexId>& source_ids, 
...
MathematicalProgramResult GraphOfConvexSets::SolveShortestPathForMultiAgent(
...
std::vector<std::vector<const Edge*>> GraphOfConvexSets::SamplePathsForAgent(
...
// Multi-agent case, for a specific agent
std::vector<const Edge*> GraphOfConvexSets::GetSolutionPathForAgent(
...
std::unique_ptr<MathematicalProgram>
GraphOfConvexSets::ConstructRestrictionProgramForAgent(
    const std::vector<const Edge*>& active_edges,
    const int agent_id,
    const MathematicalProgramResult* initial_guess) const {
  std::unique_ptr<MathematicalProgram> prog =
...
void GraphOfConvexSets::MakeRestrictionResultLookLikeMixedIntegerForAgent(
...
MathematicalProgramResult GraphOfConvexSets::SolveConvexRestrictionForAgent(
    const std::vector<const Edge*>& active_edges,
    const int agent_id, const int n_agents,
    const GraphOfConvexSetsOptions& options,
    const MathematicalProgramResult* initial_guess) const {
