...
GTEST_TEST(MultiAgentGraphOfConvexSetsTest, AddVertex) {
  MultiAgentGraphOfConvexSets g;
  ...
  const MultiAgentGraphOfConvexSets* const_g = &g;
  ...
  // For multi-agent vertex
  MultiAgentGraphOfConvexSets mult_g;
  Point p2(Vector3d(1.0, 2.0, 3.0));
  Vertex* v2 = mult_g.AddVertex(p2, "point2", 2);

  EXPECT_TRUE(mult_g.IsValid(*v2));
  EXPECT_EQ(v2->n_agents(), 2);
  EXPECT_EQ(v2->ambient_dimension(), 3);
  EXPECT_EQ(v2->full_dimension(), 6);
  EXPECT_EQ(v2->name(), "point2");

  EXPECT_EQ(mult_g.GetVertexByName("point2"), v2);
  EXPECT_EQ(mult_g.GetVertexByName("garbage"), nullptr);
  EXPECT_EQ(mult_g.GetMutableVertexByName("point2"), v2);
  EXPECT_EQ(mult_g.GetMutableVertexByName("garbage"), nullptr);

  EXPECT_EQ(v2->x().size(), 6);
  EXPECT_EQ(v2->x()[0].get_name(), "point2(0)");
  EXPECT_TRUE(v2->set().PointInSet(p2.x()));
  EXPECT_EQ(v2->x_at(1, 0), v2->x()[3]);
  EXPECT_EQ(v2->x_at(1, 0).get_name(), v2->x()[3].get_name());
}

GTEST_TEST(MultiAgentGraphOfConvexSetsTest, GetVertexSolution) {
  MultiAgentGraphOfConvexSets g;
  Point p(Vector3d(1.0, 2.0, 3.0));
  Vertex* v = g.AddVertex(p, "point");
  ...
  // For multi-agent case
  MultiAgentGraphOfConvexSets mult_g;
  Point p2(Vector3d(1.0, 2.0, 3.0));
  Vertex* v2 = mult_g.AddVertex(p2, "point2", 2);

  MathematicalProgramResult result2;
  std::unordered_map<symbolic::Variable::Id, int> map2;
  for (int i = 0; i < 6; ++i) {
...
GTEST_TEST(MultiAgentGraphOfConvexSetsTest, AddEdge) {
  ...
  Vertex* u = g.AddVertex(pu, "u", 2);
  Vertex* v = g.AddVertex(pv, "v", 2);
  Edge* e = g.AddEdge(u, v, "e", 2);
...
GTEST_TEST(MultiAgentGraphOfConvexSetsTest, RemoveEdge) {
  MultiAgentGraphOfConvexSets g;
  Point pu(Vector3d(1.0, 2.0, 3.0));
  Point pv(Vector2d(4., 5.));
  Vertex* u = g.AddVertex(pu, "u", 2);
  Vertex* v = g.AddVertex(pv, "v", 2);
  Edge* e1 = g.AddEdge(u, v, "e1", 2);
  Edge* e2 = g.AddEdge(v, u, "e2", 2);
...
GTEST_TEST(MultiAgentGraphOfConvexSetsTest, RemoveVertex) {
  MultiAgentGraphOfConvexSets g;
  Vertex* v1 = g.AddVertex(Point(Vector2d(3., 5.)), "v1", 2);
  Vertex* v2 = g.AddVertex(Point(Vector2d(-2., 4.)), "v2", 2);
  Vertex* v3 = g.AddVertex(Point(Vector2d(5., -2.3)), "v3", 2);
  Edge* e1 = g.AddEdge(v1, v2, "e1", 2);
  g.AddEdge(v1, v3, "e2", 2);
  g.AddEdge(v3, v1, "e2", 2);
